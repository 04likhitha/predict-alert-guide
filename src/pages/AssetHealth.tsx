...
import { Heart, Activity, Thermometer, Zap, Wind, Sun, Brain, Loader2, TrendingUp, TrendingDown, Clock } from 'lucide-react';
...
export default function AssetHealth() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      assets.forEach(a => {
        const d = generateSensorData(a.asset_type);
        newMap[a.asset_id] = { ...d, assetId: a.asset_id };
      });
      setSensorMap(newMap);
    }, 2000);
  ...
  const handleAIAnalysis = async (assetId: string) => {
    ...
    if (sensorData) {
      await predict('failure_prediction', sensorData, assetId);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Comprehensive health overview with AI-powered diagnostics</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {assets.map(asset => {
          const data = sensorMap[asset.asset_id];
          const health = data ? getHealthScore(data) : 100;
          const Icon = asset.asset_type === 'wind' ? Wind : Sun;
            ...
            <Card key={asset.id} className={`border-2 transition-all hover:shadow-lg ${getHealthBg(health)}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={health >= 80 ? 'default' : health >= 50 ? 'secondary' : 'destructive'}>
                    {asset.asset_id}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {data && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-1.5 p-2 rounded bg-card/50">
                      <Zap className="h-3.5 w-3.5 text-chart-1" />
                      <span className="text-muted-foreground">Power:</span>
                      <span className="font-medium ml-auto">{data.powerOutput.toFixed(0)} kW</span>
                    </div>
                    <div className="flex items-center gap-1.5 p-2 rounded bg-card/50">
                      <Clock className="h-3.5 w-3.5 text-chart-2" />
                      <span className="text-muted-foreground">RUL:</span>
                      <span className="font-medium ml-auto">{data.rulHours.toFixed(0)}h</span>
                    </div>
                    <div className="flex items-center gap-1.5 p-2 rounded bg-card/50">
                      <Thermometer className="h-3.5 w-3.5 text-chart-3" />
                      <span className="text-muted-foreground">Temp:</span>
                      <span className="font-medium ml-auto">{data.ambientTemp.toFixed(1)}°C</span>
                    </div>
                    <div className="flex items-center gap-1.5 p-2 rounded bg-card/50">
                      <Activity className="h-3.5 w-3.5 text-chart-4" />
                      <span className="text-muted-foreground">Conf:</span>
                      <span className="font-medium ml-auto">{(data.confidence * 100).toFixed(0)}%</span>
                    </div>
                  </div>
                )}
                ...
                {data && (
                  <div className="flex items-center justify-between p-2 rounded bg-card/50">
                    ...
                    <Badge variant={data.failureType === 'normal' ? 'default' : 'destructive'} className="text-xs">
                      {data.failureType === 'normal' ? 'Normal' : data.failureType.replace('_', ' ').toUpperCase()}
                    </Badge>
                  </div>
                )}
                ...
                {data && (
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span>{Math.min(100, (data.rulHours / 450) * 100).toFixed(0)}%</span>
                    </div>
                ...
                {/* AI Analysis Button */}
                <Button
                  onClick={() => handleAIAnalysis(asset.asset_id)}
                  disabled={aiLoading && selectedAsset === asset.asset_id}
                  className="w-full gap-2"
                  variant="outline"
                >
                  {aiLoading && selectedAsset === asset.asset_id ? (
                ...
                {/* AI Result */}
                {selectedAsset === asset.asset_id && aiResult && (
                  <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 text-sm space-y-2">
                    ...
                    {aiResult.risk_level && (
                      <div className="flex justify-between">
                        ...
                        <Badge variant={aiResult.risk_level === 'high' || aiResult.risk_level === 'critical' ? 'destructive' : 'default'}>
                          {aiResult.risk_level}
                        </Badge>
                      </div>
                    )}
                    {aiResult.probability !== undefined && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Failure Probability:</span>
                        <span className="font-medium">{(aiResult.probability * 100).toFixed(1)}%</span>
                      </div>
                    )}
                    {aiResult.recommendations && (
                      <div>
                        ...
                        <ul className="list-disc pl-4 text-xs mt-1 space-y-0.5">
                          {aiResult.recommendations.slice(0, 3).map((r: string, i: number) => (
                            <li key={i}>{r}</li>
                          ))}
                        </ul>
                      </div>
        ...
        {assets.length === 0 && (
          <Card className="col-span-full p-12 text-center">
            ...
            <p className="text-muted-foreground">No assets found. Add assets to see health cards.</p>
          </Card>
        )}
      </div>
    </div>
