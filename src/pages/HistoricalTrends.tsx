...
import { allAssets, getDatasetRowsForAsset } from '@/utils/datasetStreamEngine';
...
export default function HistoricalTrends() {
  ...
  const rows = getDatasetRowsForAsset(selectedAsset);
  const assetInfo = allAssets.find(a => a.id === selectedAsset);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Analyze trends from master dataset records</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4"><div className="flex items-center gap-3"><Database className="h-6 w-6 text-primary" /><div><p className="text-xs text-muted-foreground">Data Points</p><p className="text-xl font-bold">{rows.length}</p></div></div></Card>
        <Card className="p-4"><div className="flex items-center gap-3"><BarChart3 className="h-6 w-6 text-chart-2" /><div><p className="text-xs text-muted-foreground">Avg Power</p><p className="text-xl font-bold">{avgPower.toFixed(1)} kW</p></div></div></Card>
        <Card className="p-4"><div className="flex items-center gap-3"><TrendingUp className="h-6 w-6 text-success" /><div><p className="text-xs text-muted-foreground">Avg Confidence</p><p className="text-xl font-bold">{avgConf.toFixed(0)}%</p></div></div></Card>
        <Card className="p-4"><div className="flex items-center gap-3"><Clock className="h-6 w-6 text-chart-3" /><div><p className="text-xs text-muted-foreground">Avg RUL</p><p className="text-xl font-bold">{avgRul.toFixed(0)}h</p></div></div></Card>
      </div>

      {chartData.length > 0 ? (
        <div className="space-y-6">
          <Card className="card-glow p-6">
            <h3 className="text-lg font-semibold mb-4">Power Output & RUL Over Time</h3>
        ...
        <Card className="p-8 text-center text-muted-foreground">
          ...
          <p>No dataset records found for this asset.</p>
        </Card>
      )}
    </div>
  );
