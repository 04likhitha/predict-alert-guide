...
import { allAssets, getNextReading, generateAlertFromReading } from '@/utils/datasetStreamEngine';
...
export default function AlertsCenter() {
  ...
  // Dataset-driven alert generation — loops through all assets sequentially
  useEffect(() => {
    const interval = setInterval(() => {
      const asset = allAssets[assetIndexRef.current % allAssets.length];
      assetIndexRef.current++;
      const reading = getNextReading(asset.id);
      if (!reading) return;
      const alert = generateAlertFromReading(reading);
      if (alert) {
        setSessionAlerts(prev => [alert, ...prev]);
        supabase.from('alerts_history').insert({
          asset_id: alert.assetId,
          severity: alert.severity,
          message: alert.message,
          failure_type: reading.failureType,
          rul_hours: reading.rulHours,
        }).then(() => {});
      }
    }, 3000);
  ...
  const allAlerts = [
    ...sessionAlerts.map(a => ({ id: a.id, asset_id: a.assetId, severity: a.severity, message: a.message, created_at: a.timestamp.toISOString(), acknowledged: false, resolved: false, isSession: true })),
    ...dbAlerts.map(a => ({ ...a, isSession: false })),
  ];

  const filtered = filter === 'all' ? allAlerts : filter === 'unresolved' ? allAlerts.filter(a => !a.resolved) : filter === 'session' ? allAlerts.filter(a => a.isSession) : allAlerts.filter(a => a.severity === filter);

  const severityIcons: Record<string, any> = { critical: XCircle, high: AlertCircle, medium: AlertTriangle, low: Info };
  const severityStyles: Record<string, string> = { critical: 'border-destructive/30 bg-destructive/5', high: 'border-destructive/20 bg-destructive/5', medium: 'border-warning/20 bg-warning/5', low: 'border-primary/20 bg-primary/5' };
  const counts = { total: allAlerts.length, critical: allAlerts.filter(a => a.severity === 'critical').length, unresolved: allAlerts.filter(a => !a.resolved).length, session: sessionAlerts.length };

  return (
    <div className="p-6 max-w-[1800px] mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="page-header mb-0"><h1>Alerts Center</h1><p>Dataset-driven alert tracking — session-persistent</p></div>
        <div className="flex gap-1.5 flex-wrap">
          {['all', 'critical', 'high', 'medium', 'low', 'unresolved', 'session'].map(f => (
            <Button key={f} size="sm" variant={filter === f ? 'default' : 'outline'} onClick={() => setFilter(f)} className="capitalize text-[11px] h-7">{f}</Button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <AnimatePresence>
          {filtered.slice(0, 50).map((alert, i) => {
            ...
            return (
              <motion.div key={alert.id + '-' + i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2, delay: i * 0.02 }}>
        ...
        {filtered.length === 0 && <Card className="p-8 text-center border border-border/50"><Bell className="h-8 w-8 mx-auto mb-3 text-muted-foreground/30" /><p className="text-xs text-muted-foreground">No alerts match the current filter</p></Card>}
      </div>
    </div>
  );
}
