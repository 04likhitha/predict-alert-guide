  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [{ data: t }, { data: a }] = await Promise.all([
        ...
        supabase.from('assets').select('asset_id, name'),
      ]);
      if (t) setTasks(t);
      if (a) setAssets(a);
      setLoading(false);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold glow-text">Maintenance Planner</h1>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <Select value={form.asset_id} onValueChange={v => setForm({ ...form, asset_id: v })}>
                ...
                <SelectContent>
                  {assets.map(a => <SelectItem key={a.asset_id} value={a.asset_id}>{a.name} ({a.asset_id})</SelectItem>)}
                </SelectContent>
              </Select>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {Object.entries(counts).map(([status, count]) => {
          ...
          return (
            <Card key={status} className={`p-4 cursor-pointer ${filter === status ? 'ring-2 ring-primary' : ''}`}
      ...
      <div className="space-y-3">
        ...
        {filtered.length === 0 && !loading && (
          <Card className="p-8 text-center text-muted-foreground">No tasks found</Card>
        )}
      </div>
    </div>
  );
