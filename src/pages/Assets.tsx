...
import { Wind, Sun, Activity, Zap, Thermometer, Droplets, MapPin } from 'lucide-react';
...
export default function Assets() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      allAssets.forEach(asset => {
        ...
        if (reading) newMap[asset.id] = reading;
      });
      setSensorMap(newMap);
    };
    tick();
  ...
  const AssetCard = ({ asset, data, icon: Icon }: { asset: DatasetAsset; data: SensorData | undefined; icon: any }) => (
    <Card className="hover:shadow-lg transition-all duration-300 border-border/50">
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span className="ml-auto">{asset.capacity_mw} MW</span>
        </div>
          ...
          <>
            ...
            {data.assetType === 'wind' && (
              <div className="pt-2 border-t border-border/50 space-y-1">
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Wind Speed:</span><span className="font-medium">{data.windSpeed?.toFixed(1)} m/s</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Rotor Speed:</span><span className="font-medium">{data.rotorSpeed?.toFixed(1)} rpm</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Gearbox Temp:</span><span className="font-medium">{data.gearboxTemp?.toFixed(1)}°C</span></div>
              </div>
            )}

            {data.assetType === 'solar' && (
              <div className="pt-2 border-t border-border/50 space-y-1">
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Voltage:</span><span className="font-medium">{data.panelVoltage?.toFixed(1)} V</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Current:</span><span className="font-medium">{data.panelCurrent?.toFixed(2)} A</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Irradiance:</span><span className="font-medium">{data.irradiance?.toFixed(0)} W/m²</span></div>
              </div>
            )}
            ...
            <div className="pt-2 border-t border-border/50">
              ...
              <div className="w-full bg-secondary rounded-full h-1.5 mt-1">
                <div className="bg-primary h-1.5 rounded-full transition-all duration-300" style={{ width: `${data.confidence * 100}%` }} />
              </div>
            </div>
          </>
        )}
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground">All assets sourced from Solar & Wind Master Datasets — live loop streaming</p>
      </div>

      <Tabs defaultValue="wind" className="w-full">
        <TabsList className="grid w-full max-w-md grid-cols-2">
          <TabsTrigger value="wind" className="flex items-center gap-2"><Wind className="h-4 w-4" /> Wind Turbines ({windAssetsList.length})</TabsTrigger>
          <TabsTrigger value="solar" className="flex items-center gap-2"><Sun className="h-4 w-4" /> Solar Panels ({solarAssetsList.length})</TabsTrigger>
        </TabsList>

        <TabsContent value="wind" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {windAssetsList.map(asset => (
